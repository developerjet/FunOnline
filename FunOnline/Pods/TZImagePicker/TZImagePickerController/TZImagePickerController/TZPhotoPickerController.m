...
- (void)configCollectionView {
  ...
  _collectionView = [[UICollectionView alloc]
             initWithFrame:CGRectMake(margin, top,
                                      self.view.tz_width - 2 * margin,
  ...
  _collectionView.contentSize = CGSizeMake(
      self.view.tz_width, ((_model.count + 3) / 4) * self.view.tz_width);
  [self.view addSubview:_collectionView];
  [_collectionView registerNib:[UINib nibWithNibName:@"TZAssetCell" bundle:nil]
  ...
  if (_shouldScrollToBottom && _photoArr.count > 0) {
    [_collectionView
        scrollToItemAtIndexPath:[NSIndexPath
                                    indexPathForItem:(_photoArr.count - 1)
                                           inSection:0]
               atScrollPosition:UICollectionViewScrollPositionBottom
                       animated:NO];
    _shouldScrollToBottom = NO;
  }
  // Determine the size of the thumbnails to request from the
...
- (void)configBottomToolBar {
  UIView *bottomToolBar =
      [[UIView alloc] initWithFrame:CGRectMake(0, self.view.tz_height - 50,
                                               self.view.tz_width, 50)];
  ...
  if (imagePickerVc.allowPickingOriginalPhoto) {
    ...
    _originalPhotoButton.frame =
        CGRectMake(50, self.view.tz_height - 50, 130, 50);
    ...
    [_originalPhotoButton setImage:[UIImage imageNamed:@"photo_original_def"]
                          forState:UIControlStateNormal];
    [_originalPhotoButton setImage:[UIImage imageNamed:@"photo_original_sel"]
                          forState:UIControlStateSelected];
  ...
  [_okButton setTitleColor:imagePickerVc.oKButtonTitleColorNormal
                  forState:UIControlStateNormal];
  [_okButton setTitleColor:imagePickerVc.oKButtonTitleColorDisabled
                  forState:UIControlStateDisabled];
  _okButton.enabled = NO;

  _numberImageView = [[UIImageView alloc]
      initWithImage:[UIImage imageNamed:@"photo_number_icon"]];
  ...
  _numberLable.text =
      [NSString stringWithFormat:@"%zd", _selectedPhotoArr.count];
...
- (void)cancel {
  ...
  TZImagePickerController *imagePickerVc =
      (TZImagePickerController *)self.navigationController;
  if ([imagePickerVc.pickerDelegate
        ...
        getPhotoWithAsset:model.asset
               completion:^(UIImage *photo, NSDictionary *info,
                 ...
                 if ([imagePickerVc.pickerDelegate
                         respondsToSelector:@selector(
                                                ...
                                                didFinishPickingPhotos:
                                                          sourceAssets:)]) {
                   [imagePickerVc.pickerDelegate
                        imagePickerController:imagePickerVc
                 ...
                 if (imagePickerVc.didFinishPickingPhotosWithInfosHandle) {
                   imagePickerVc.didFinishPickingPhotosWithInfosHandle(
                       photos, assets, infoArr);
                 }
  ...
  cell.didSelectPhotoBlock = ^(BOOL isSelected) {
    ...
    if (isSelected) {
      ...
      if (weakSelf.selectedPhotoArr.count < imagePickerVc.maxImagesCount) {
        ...
        [imagePickerVc
            showAlertWithTitle:
                [NSString stringWithFormat:@"你最多只能选择%zd张照片",
                                           imagePickerVc.maxImagesCount]];
      }
    }
    [UIView showOscillatoryAnimationWithLayer:weakLayer
                                         type:TZOscillatoryAnimationToSmaller];
  };
  return cell;
}
    ...
    if (_selectedPhotoArr.count > 0) {
      ...
      if (_shouldPreviewVideo == YES) {
        ...
        if (iOS8Later) {
          ...
          [[PHImageManager defaultManager]
              requestAVAssetForVideo:model.asset
                       ...
                       resultHandler:^(AVAsset *_Nullable asset,
                         ...
                         CGImageRef cfImage =
                             [generator copyCGImageAtTime:kCMTimeZero
                                               actualTime:nil
                                                    error:nil];
                         UIImage *image = [UIImage imageWithCGImage:cfImage];
                         if ([imagePickerVc.pickerDelegate
                                 respondsToSelector:
                           ...
                           [imagePickerVc.pickerDelegate
                               imagePickerController:imagePickerVc
                               didFinishPickingVideo:image
                                        sourceAssets:model.asset];
                         }
          ...
          NSURL *url = [(ALAsset *)(model.asset)defaultRepresentation].url;
          AVAsset *asset = [AVAsset assetWithURL:url];
          AVAssetImageGenerator *generator =
              [AVAssetImageGenerator assetImageGeneratorWithAsset:asset];
          CGImageRef cfImage = [generator copyCGImageAtTime:kCMTimeZero
                                                 actualTime:nil
                                                      error:nil];
          UIImage *image = [UIImage imageWithCGImage:cfImage];
          if ([imagePickerVc.pickerDelegate
                  respondsToSelector:@selector(imagePickerController:
                                               didFinishPickingVideo:
                                                        sourceAssets:)]) {
            [imagePickerVc.pickerDelegate imagePickerController:imagePickerVc
                                          didFinishPickingVideo:image
                                                   sourceAssets:model.asset];
          }
          if (imagePickerVc.didFinishPickingVideoHandle) {
...
- (void)pushPhotoPrevireViewController:
  ...
  photoPreviewVc.returnNewSelectedPhotoArrBlock =
      ^(NSMutableArray *newSelectedPhotoArr, BOOL isSelectOriginalPhoto) {
        _selectedPhotoArr = newSelectedPhotoArr;
        _isSelectOriginalPhoto = isSelectOriginalPhoto;
        [_collectionView reloadData];
        [self refreshBottomToolBarStatus];
      };
  photoPreviewVc.okButtonClickBlock =
      ^(NSMutableArray *newSelectedPhotoArr, BOOL isSelectOriginalPhoto) {
        _selectedPhotoArr = newSelectedPhotoArr;
        _isSelectOriginalPhoto = isSelectOriginalPhoto;
        [self okButtonClick];
...
- (void)getSelectedPhotoBytes {
  [[TZImageManager manager]
      getPhotosBytesWithArray:_selectedPhotoArr
...
- (NSArray *)aapl_indexPathsForElementsInRect:(CGRect)rect {
  ...
  for (UICollectionViewLayoutAttributes
           *layoutAttributes in allLayoutAttributes) {
